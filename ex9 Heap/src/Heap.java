public class Heap {
	...
	public static boolean isAHeap(Heap h) {
		for (int parent = 0; parent < h.size/2; parent++) {
			int leftchild = (2 * parent) + 1;
			int rightchild = (2 * parent) + 2;
			System.out.print(" test"+"last leaf: "+(h.size-1)+" lc:"+leftchild+" rc:"+rightchild);
			
			if (h.mData[parent] > h.mData[leftchild]) {
				return false;
			}
			
			if (rightchild < (h.size-1)) {
				if (h.mData[parent] > h.mData[rightchild]) {
					return false;
				}
			}
