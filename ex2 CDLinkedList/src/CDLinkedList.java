public class CDLinkedList {
	...
	public void swapRange(Iterator s1, Iterator f1, Iterator s2, Iterator f2) throws Exception {
		// null and not DListIterator
		if (!(s1 instanceof DListIterator) || !(s2 instanceof DListIterator) || !(f1 instanceof DListIterator) || !(f2 instanceof DListIterator) || s1 == null || s2 == null || f1 == null || f2 == null){
            return;
        }
		
		// Iterator 
        DListIterator st1 = (DListIterator) s1;
        DListIterator st2 = (DListIterator) s2;
        DListIterator fl1 = (DListIterator) f1;
        DListIterator fl2 = (DListIterator) f2;
        
        //swap start
        DListIterator st1_previous = (DListIterator) findPrevious(st1);
        DListIterator st2_previous = (DListIterator) findPrevious(st2);
        
        // st2prev <-- st1  && st2prev --> st1
        st1.currentNode.previousNode = st2_previous.currentNode;
        st2_previous.currentNode.nextNode = st1.currentNode;
        
        // st1prev <-- st2  && st1prev --> st2
        st2.currentNode.previousNode = st1_previous.currentNode;
        st1_previous.currentNode.nextNode = st2.currentNode;
        
        //swap end
        DListIterator fl1_next = new DListIterator (fl1.currentNode.nextNode);
        DListIterator fl2_next = new DListIterator (fl2.currentNode.nextNode);
        
        // fl1 --> fl2next && fl1 <-- fl2next
        fl1.currentNode.nextNode = fl2_next.currentNode;
        fl2_next.currentNode.previousNode = fl1.currentNode;
        
        // fl2 --> fl1next && fl2 <-- fl1next
        fl2.currentNode.nextNode = fl1_next.currentNode;
        fl1_next.currentNode.previousNode = fl2.currentNode;
